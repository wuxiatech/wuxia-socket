...
Protocol.GROUP_ACK_WINDOW   = "group_ack_window";
Protocol.GROUP_ACK_TIMEOUT  = "group_ack_timeout";
...
Protocol.MASK_ACK_REQUIRED   = 1<<6;  
...
function httpEncode(msg) {
    ...
    for (var key in msg) {
        ...
        var val = msg[key];
        //key = camel2underscore(key); //TODO
        if (typeof val == 'undefined' || val == null) continue;
        if(Message.codecHeaders.has(key)){
...
function httpDecode(data) {
    ...
    var msg = {};
    //parse first line 
    var firstLine = decodeURIComponent(lines[0]);
    var bb = firstLine.split(" ");
...
MqClient.prototype.ack = function (msg) {
    var res = {};
    res.token = this.token; 
    res.ack = false;
    res.cmd = Protocol.ACK; 
    res.offset = msg.offset;
    res.topic = msg.topic;
    res.consume_group = msg.consume_group; 
...
class Consumer extends MqAdmin {
    constructor(broker, msgCtrl){
        super(broker, msgCtrl.token); 
        this.consumeTimeout = 30*1000; //30s
        if (msgCtrl.constructor == String) {
            msgCtrl = {
                topic: msgCtrl,
                consume_group: msgCtrl,
                token: this.token
            };
        }

        this.declareMessage = clone(msgCtrl);
        this.consumeMessage = {
            topic: msgCtrl.topic,
            consume_group: msgCtrl.consume_group,
            token: msgCtrl.token
        };

        if(!this.declareMessage.consume_group && !this.declareMessage.group_name_auto){
            this.consumeMessage.consume_group = this.consumeMessage.topic;
        }
    ...
    _startConsume(client){
        var consumer = this;   
        var declareCtrl = clone(this.declareMessage);
        if(declareCtrl.token) client.token = declareCtrl.token;

        client.declare(declareCtrl)
            .then(res=>{
                if (res.error) { 
                    throw new Error("declare error: " + res.error);
                } 
                this.consumeMessage.consume_group = res.groupName;
    ...
    consume(client) {  
        var consumer = this;   
        var consumeMessage = clone(consumer.consumeMessage); 

        if(this.consumeTimer != null){
            clearInterval(this.consumeTimer); 
            this.consumeTimer = null;
        }
        this.consumeTimer = setInterval(()=>consumer.consume(client), consumer.consumeTimeout);

        client.consume(consumeMessage)
            .then(res => {
                if(res.status == 404) {//Missing topic, to declare 
                    var ctrl = clone(consumer.declareMessage);
                    ...
                    return client.declare(ctrl)
                        .then(res=>{
                            ...
                            consumer.consumeMessage.consume_group = res.groupName;
                            return consumer.consume(client);
                        });
                } 
                if(res.status != 200){ 
...
function buildModuleInfo(rpc, module){ 
    var moduleInfo = '';
    var topic = rpc.docContext;
    var moduleMethods = rpc.modules[module];
    for(var methodName in moduleMethods){
        var m = moduleMethods[methodName]; 
        var args = m['params']; 
        moduleInfo +=  RpcModuleTemplate.format(topic, module, methodName, args);
    }  
    return moduleInfo;
} 
...
function buildRpcInfo(rpc, module=null){ 
    var modules_info = '';
    if(module == null){
        for(var module_name in rpc.modules){
            modules_info += buildModuleInfo(rpc, module_name);
        } 
    } else { 
        modules_info = buildModuleInfo(rpc, module);
    }
    return RpcInfoTemplate.format(rpc.docContext, RpcStyleTemplate, modules_info);
}

class RpcModuleInfo {
    constructor(rpcProcessor, module){
...
class RpcProcessor {
    constructor(){
        ...
        this.messageHandler = function (msg, client) { 
            ...
            try { 
                ...
                if(!req.module) req.module = "index";
                if(!req.method) req.method = "index";
                ...
                if(result != null && result.constructor == Message){
                    result.id = resMsg.id;
                    ...
                    resMsg['content-type'] = 'application/json';
                    resMsg.body = result;
                }
    ...
    enableDoc(value){
        if(!value) return; 
        var indexMethod = this._generateKey("index", "index");
        if(indexMethod in this.methodTable){
            return;
        }

        for(var module in this.modules){
            var key = this._generateKey(module, "index");
            if(this.methodTable[key] == null) {
                var moduleInfoObject = new RpcModuleInfo(this, module);
                this.methodTable[key] = { method: moduleInfoObject.index, target: moduleInfoObject };
            }
        }
    ...
    addModule(moduleName, serviceObj) {
        if(typeof serviceObj == 'string' && typeof moduleName != 'string'){ //for compatiable reason
            var temp = moduleName;
            moduleName = serviceObj;
            serviceObj = temp;
        }
        ...
        if (typeof (serviceObj) == 'function') {
            ...
            moduleInfo[func.name] = func;
            return;
        }
        var methods = this._getAllMethods(serviceObj);
        for (var name of methods) { 
            var func = serviceObj[name]; 
            var key = this._generateKey(moduleName, name);
            if (key in this.methodTable) {
                logger.warn(key + " already exists, will be overwrite");
            }
            this.methodTable[key] = { method: func, target: serviceObj };
            var paramString = this._getFnParamNames(func.toString());
            moduleInfo[func.name] = {method: name, params: paramString};
        }
    }
...
class ServiceBootstrap {
    constructor(){
        ...
        this.serviceMask = Protocol.MASK_MEMORY | Protocol.MASK_RPC;
    }  
    ...
    serviceMask(mask){
        this.serviceMask = mask | Protocol.MASK_RPC;
    }
    ...
    addModule(moduleName, serviceObj){
        this.processor.addModule(moduleName, serviceObj);
        return this;
    } 

    enableDoc(value){
        this.processor.enableDoc(value);
        return this;
    } 
}
